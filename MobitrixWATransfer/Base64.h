...
public:
    ...
    static bool EncodeStr(const char* input, int input_length, std::string& outstr) {
        //
        size_t encoded_length = EncodedLength(input_length);
        int i = 0, j = 0;
        ...
        unsigned char a3[3];
        unsigned char a4[4];
        char* out = new char[encoded_length + 10];
        char* out_begin=out;
        while (input_length--) {
            a3[i++] = *input++;
            if (i == 3) {
        ...
        out_begin[encoded_length] = 0;
        outstr = std::string(out_begin);
        delete[] out_begin;
        return true;
    }
